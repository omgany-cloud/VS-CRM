...
/* ===== ONBOARDING TABLE ===== */
function renderOnboardingTable(data) {
  // Update funnel counts (guard: elements may not exist if page-onboarding removed)
  const fnP = document.getElementById('fn-prospect'); if (fnP) fnP.textContent = lpList.filter(l => l.status === 'Потенциальный').length;
  const fnK = document.getElementById('fn-kyc');      if (fnK) fnK.textContent = lpList.filter(l => l.status === 'KYC в процессе').length;
  const fnS = document.getElementById('fn-sub');      if (fnS) fnS.textContent = lpList.filter(l => l.status === 'Subscription Agreement').length;
  const fnA = document.getElementById('fn-active');   if (fnA) fnA.textContent = lpList.filter(l => l.status === 'Активный LP').length;
  ...
  tbody.innerHTML = data.map(lp => `
    <tr>
      <td>
        <div class="cell-name">
          <div class="cell-avatar" style="background:${getColor(lp.id)}">${lp.name.charAt(0)}</div>
          <div class="cell-name-text">
            <strong>${lp.name}</strong>
            <span>${lp.country} · ${lp.contact}</span>
          </div>
        </div>
      </td>
      <td>${lp.type}</td>
      <td><strong style="color:var(--text-primary)">$${lp.commit}M</strong></td>
      <td>${onbStatusBadge(lp.status)}</td>
      <td>${kycStatusBadge(lp.kyc.status)}</td>
      <td>${lp.subAgreement
            ? `<span class="badge badge-green"><i class="fas fa-check"></i> Подписан ${lp.subDate ? formatDate(lp.subDate) : ''}</span>`
            : `<span class="badge badge-gray">Не подписан</span>`}</td>
      <td><span style="font-size:12px;color:var(--text-muted)">${lp.manager}</span></td>
      <td>
        <div class="action-btns">
          <button class="act-btn" title="Обновить KYC" onclick="openKYCModal(${lp.id})"><i class="fas fa-shield-alt"></i></button>
          <button class="act-btn" title="Продвинуть статус" onclick="advanceOnboarding(${lp.id})"><i class="fas fa-forward"></i></button>
          <button class="act-btn del" title="Удалить" onclick="deleteLP(${lp.id})"><i class="fas fa-trash"></i></button>
        </div>
      </td>
    </tr>
  `).join('');
...
function filterOnboarding(search) {
  const q = (search || document.getElementById('searchOnboarding').value || '').toLowerCase();
  const st = document.getElementById('filterOnbStatus').value;
  const ty = document.getElementById('filterOnbType').value;
  const filtered = lpList.filter(lp => {
    const mq = !q || lp.name.toLowerCase().includes(q);
    const ms = !st || lp.status === st;
    const mt = !ty || lp.type === ty;
    return mq && ms && mt;
  });
  renderOnboardingTable(filtered);
}

function advanceOnboarding(id) {
  const lp = lpList.find(l => l.id === id);
  if (!lp) return;
  const flow = ['Потенциальный','KYC в процессе','Subscription Agreement','Активный LP'];
  const idx = flow.indexOf(lp.status);
  if (idx < flow.length - 1) {
    lp.status = flow[idx + 1];
    if (lp.status === 'Subscription Agreement' && lp.kyc.status !== 'Одобрен') {
      showToast('⚠️ KYC не одобрен — сначала завершите KYC', 'orange');
      lp.status = flow[idx];
      return;
    }
    if (lp.status === 'Активный LP') { lp.subAgreement = true; lp.invested = lp.commit; }
    renderOnboardingTable(lpList);
    renderKYCTable(lpList);
    updateBadges();
    showToast(`✅ ${lp.name} → ${lp.status}`);
  } else {
    showToast('ℹ️ LP уже активен', 'blue');
  }
}

function deleteLP(id) {
  if (!confirm('Удалить LP из системы?')) return;
  lpList = lpList.filter(l => l.id !== id);
  renderOnboardingTable(lpList);
  renderKYCTable(lpList);
  updateBadges();
  renderDashboard();
  showToast('🗑️ LP удалён', 'red');
}
...
function renderKYCTable(data) {
  ...
  tbody.innerHTML = data.map(lp => {
    const fields = ['passport','proofAddress','sourceOfFunds','taxId','pepCheck','amlScreening'];
    return `
      <tr>
        <td>
          <div class="cell-name">
            <div class="cell-avatar" style="background:${getColor(lp.id)}">${lp.name.charAt(0)}</div>
            <div class="cell-name-text"><strong>${lp.name}</strong><span>${lp.country}</span></div>
          </div>
        </td>
        <td style="font-size:12px;color:var(--text-muted)">${lp.type}</td>
        ${fields.map(f => `<td>${checkIcon(lp.kyc[f])}</td>`).join('')}
        <td>${kycStatusBadge(lp.kyc.status)}</td>
        <td>
          <div class="action-btns">
            <button class="act-btn" title="Обновить KYC" onclick="openKYCModal(${lp.id})"><i class="fas fa-edit"></i></button>
          </div>
        </td>
      </tr>
    `;
...
function filterKYC(search) {
  const q = (search || document.getElementById('searchKYC').value || '').toLowerCase();
  const st = document.getElementById('filterKYCStatus').value;
  const filtered = lpList.filter(lp => {
    const mq = !q || lp.name.toLowerCase().includes(q);
    const ms = !st || lp.kyc.status === st;
    return mq && ms;
  });
...
function openKYCModal(id) {
  const lp = lpList.find(l => l.id === id);
  if (!lp) return;
  document.getElementById('kyc_lp_id').value = id;
  document.getElementById('kyc_status_select').value = lp.kyc.status;
  document.getElementById('kyc_date').value = lp.kyc.date || '';
  document.getElementById('kyc_comment').value = lp.kyc.comment || '';

  // Build KYC checklist form
  const fields = lp.type === 'Физическое лицо' ? KYC_FIELDS_INDIVIDUAL : KYC_FIELDS_ENTITY;
  const container = document.getElementById('kycChecklistForm');
  container.innerHTML = fields.map(f => `
    <label class="kyc-check-row">
      <input type="checkbox" id="kyc_field_${f.key}" ${lp.kyc[f.key] ? 'checked' : ''} />
      <span class="kyc-check-label">${f.label}</span>
    </label>
  `).join('');
...
function saveKYC() {
  const id = parseInt(document.getElementById('kyc_lp_id').value);
  const lp = lpList.find(l => l.id === id);
  if (!lp) return;

  const fields = lp.type === 'Физическое лицо' ? KYC_FIELDS_INDIVIDUAL : KYC_FIELDS_ENTITY;
  fields.forEach(f => {
    const el = document.getElementById('kyc_field_' + f.key);
    if (el) lp.kyc[f.key] = el.checked;
  });

  lp.kyc.status  = document.getElementById('kyc_status_select').value;
  lp.kyc.date    = document.getElementById('kyc_date').value;
  lp.kyc.comment = document.getElementById('kyc_comment').value;
  ...
  updateBadges();
  closeModal();
  showToast(`✅ KYC обновлён: ${lp.name} → ${lp.kyc.status}`);
}
...
function saveLP() {
  ...
  const country   = document.getElementById('lp_country').value.trim() || 'Казахстан';
  const commit    = parseFloat(document.getElementById('lp_commit').value) || 0;
  const contact   = document.getElementById('lp_contact').value.trim() || '—';
  const email     = document.getElementById('lp_email').value.trim() || '';
  const phone     = document.getElementById('lp_phone').value.trim() || '';
  const manager   = document.getElementById('lp_manager').value;
  const status    = document.getElementById('lp_status').value;
  const qualified = document.getElementById('lp_qualified').value;

  if (!name) { alert('Введите название / ФИО'); return; }
  if (commit < FUND_PARAMS.minCommitment) {
    if (!confirm(`Минимальный commitment $${FUND_PARAMS.minCommitment}M. Продолжить?`)) return;
  }

  const newLP = {
    id: Date.now(), name, type, country, commit, invested: 0, status, manager,
    contact, email, phone, qualified,
    kyc: { status:'Не начат', passport:false, proofAddress:false, sourceOfFunds:false, taxId:false, pepCheck:false, amlScreening:false, uboVerified:false, date:null, comment:'' },
    subAgreement: false, subDate: null, capitalCalled: 0, distributions: 0,
  };
  lpList.push(newLP);
  todayTasks.push({
    id: Date.now(), done: false, priority: 'Высокий',
    text: `Начать KYC для нового LP: ${name}`, page: 'kyc'
  });
  ...
  renderDashboard();
  closeModal();
  showToast(`✅ LP добавлен: ${name}`);
  ['lp_name','lp_country','lp_commit','lp_contact','lp_email','lp_phone'].forEach(id => document.getElementById(id).value = '');
}
...
function checkIcon(v) {
  return v
    ? '<span style="color:var(--accent-green);font-size:16px"><i class="fas fa-check-circle"></i></span>'
    : '<span style="color:var(--accent-red);font-size:16px"><i class="fas fa-times-circle"></i></span>';
}

function onbStatusBadge(s) {
  const m = { 'Активный LP':'badge-green','KYC в процессе':'badge-orange','Subscription Agreement':'badge-purple','Потенциальный':'badge-blue' };
