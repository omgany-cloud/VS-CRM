...
function rowToRole(row) {
  return {
    ...
    afsaSubmit: !!row.afsa_submit,
    aiAssist: !!row.ai_assist,
    screeningAccept: !!row.screening_accept,
    icSeat: row.ic_seat,
...
const NO_PERMISSIONS = {
  ...
  amlClear: false, ccApprove: false, paymentConfirm: false, afsaSubmit: false, aiAssist: false, screeningAccept: false, icSeat: null,
};

const INSERT_SQL = `
  INSERT INTO roles
    (tenant_id, code, label, icon, color, internal, manage_users, manage_roles,
     access_fm, decide_conflicts, author_ic_memo, risk_veto, read_only, aml_clear, cc_approve, payment_confirm, afsa_submit, ai_assist, screening_accept, ic_seat, is_system)
  VALUES
    (@tenantId, @code, @label, @icon, @color, @internal, @manageUsers, @manageRoles,
     @accessFM, @decideConflicts, @authorICMemo, @riskVeto, @readOnly, @amlClear, @ccApprove, @paymentConfirm, @afsaSubmit, @aiAssist, @screeningAccept, @icSeat, @isSystem)
`;
...
const UPDATE_SQL = `
  UPDATE roles SET
    ...
    payment_confirm=@paymentConfirm, afsa_submit=@afsaSubmit, ai_assist=@aiAssist, screening_accept=@screeningAccept, ic_seat=@icSeat
...
function roleToParams(r) {
  return {
    ...
    afsaSubmit: r.afsaSubmit ? 1 : 0,
    aiAssist: r.aiAssist ? 1 : 0,
    screeningAccept: r.screeningAccept ? 1 : 0,
